...
class Score : public Text {
    private:
        ...
        std::string mainText;
    public:
        inline Score(const std::string& text, const SDL_Rect& rect, TextMode mode, SDL_Color color, const std::string& fontPath, int fontSize)
            : Text(text + std::to_string(0), rect, 0, mode, color, fontPath, fontSize), score(0) 
        {
            mainText = text;
        }
        ...
        inline void changeFontPath(const std::string& new_font) { Text::changeFontPath(new_font); }
